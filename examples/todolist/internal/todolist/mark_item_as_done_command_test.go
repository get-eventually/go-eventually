...
func TestMarkItemAsDoneCommandHandler(t *testing.T) {
	now := time.Now()
	commandHandlerFactory := func(es event.Store) todolist.MarkItemAsDoneCommandHandler {
	...
	t.Run("it fails when the target TodoList does not exist", func(t *testing.T) {
		command.Scenario[todolist.MarkItemAsDoneCommand, todolist.MarkItemAsDoneCommandHandler]().
			When(command.ToEnvelope(todolist.MarkItemAsDoneCommand{
