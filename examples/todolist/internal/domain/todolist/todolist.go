...
// String returns the canonical UUID string representation of the ID.
...
func (tl *TodoList) AggregateID() ID { return tl.ID }
...
func (tl *TodoList) applyItemEvent(id ItemID, evt event.Event) error {
	...
	if !ok {
		return errors.New("todolist.TodoList.Apply: item not found")
	}
...
func (tl *TodoList) Apply(evt event.Event) error {
	switch evt := evt.(type) {
	case WasCreated:
		tl.ID = evt.ID
		tl.Title = evt.Title
	...
	case ItemWasDeleted:
		items := make([]*Item, 0, len(tl.Items))
